...
export class KimiAdapter extends BaseAdapter {
  ...
  getUserMessages(): QuestionItem[] {
    ...
    validElements.forEach((el) => {
      // 2. 尝试定位文本内容节点
      const contentEl = (el.querySelector('[class*="content"], [class*="text"]') as HTMLElement) || el;
      
      // 3. 克隆节点并剔除“编辑”、“复制”、“分享”等按钮和工具栏节点
      const clone = contentEl.cloneNode(true) as HTMLElement;
      clone.querySelectorAll('[class*="action"], [class*="tool"], [class*="btn"], button, [class*="ops"]').forEach(node => node.remove());
